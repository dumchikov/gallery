...
if ($webConfig -eq $null) { # Not a web project
	...
	$codePath = Join-Path (Split-Path $project.FullName -Parent) "App_Start\$($profiler)ProfilerProductionBootstrapper.cs"
	(Get-Content $codePath) | Where-Object {$_ -notlike "*assembly: WebActivator.PreApplicationStartMethod*"} | Set-Content $codePath
	$codePath = Join-Path (Split-Path $project.FullName -Parent) "App_Start\$($profiler)ProfilerProductionBootstrapper.vb"
	(Get-Content $codePath) | Where-Object {$_ -notlike "*assembly: WebActivator.PreApplicationStartMethod*"} | Set-Content $codePath
